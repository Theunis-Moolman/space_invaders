...
class Player:
    ...
    def line_rotate(
        self, is_clockwise: bool, counterclockwise: bool, rotation_speed: float
    ):
        # Rotate left
        if counterclockwise:
    ...
    def draw_spaceship(self, line_length, change_color):
        ...
        stddraw.line(
            (self.x + 1) / 2, (self.y + 1) / 2, (end_x + 1) / 2, (end_y + 1) / 2
        )  # Shows where you are aiming
    ...
    def draw_projectiles(self) -> None:
        for projectile in self.projectiles:
            ...
            stddraw.picture(
                Picture("assets/images/Projectile.png"), projectile.x, projectile.y
            )
    ...
    def check_hit_laser(self, enemy):
        ...
        if (
            (self.x + 1) / 2 - self.radius * 0.65
            <= enemy.x
            <= (self.x + 1) / 2 + self.radius * 0.65
        ):
            if (
                time.time() - self.shield_timer > self.shield_cooldown and not self.hit
            ):  # If not shielded subtract a life
                self.hit = True
                self.lives -= 1
    ...
    def check_hit_projectile(self, projectiles):
        projectiles_to_remove = (
            []
        )  # Array to make sure projectiles are not removed while iterating
        for projectile in projectiles[:]:
            # Check distance to determine a hit
            if (
                math.hypot(
                    (self.x + 1) / 2 - projectile.x, (self.y + 1) / 2 - projectile.y
                )
                < self.radius * 0.7
            ):
                if time.time() - self.shield_timer > self.shield_cooldown:
                    self.lives -= 1
                projectiles_to_remove.append(
                    projectile
                )  # Remove regardless of hit or shield hit
